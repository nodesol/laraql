...
- `#[QueryCollection]`: `name` = `Str::snake(Str::plural(class))`, `filters` = `['where: _ @whereConditions(column: {})', 'first: Int! = 10', 'page: Int', 'orderBy: _ @orderBy']`, `return_type` = `[Type!]!` (Lighthouse turns that into `<Type>Paginator` because of `@paginate`), `query` = `'@paginate(defaultCount: 10)'`. Every orderable relation is added as its own `orderBy<Relation>` argument (`orderByTenant` for a `tenant()` relation) so the default `orderBy` argument keeps the shared `OrderByClause` type; set `order_by_relations: false` to leave the extra arguments out.
